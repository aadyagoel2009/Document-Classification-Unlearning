...
def main(unlearned_class):
    ...
    class_importance = zero_class_importance(class_importance, unlearned_class)
    ...
    y_pred = [classify_document(doc) for doc in test_dataset]
    accuracy_all = accuracy_score(y_test_labels, y_pred)
    print(f"Accuracy on test data after unlearning {unlearned_class}: {accuracy_all * 100:.2f}%")

    # Accuracy excluding the unlearned class
    valid_indices = [i for i, label in enumerate(y_test_labels) if label != unlearned_class]
    ...
    print(f"Accuracy excluding class {unlearned_class}: {accuracy_excl * 100:.2f}%")
    

    # Distribution of classifications for the unlearned class
    unlearned_class_indices = [i for i, label in enumerate(y_test_labels) if label == unlearned_class]
    ...
    print(f"Distribution of classifications for class {unlearned_class} (as percentages):", distribution_percent)

    return distribution_percent

def gold_standard(unlearned_class):
    """Runs the model without the unlearned class in training, creating a fair baseline."""
    
    dataset = load_20newsgroup_dataset()
    documents = dataset.data
    labels = dataset.target

    # Separate documents based on whether they belong to the unlearned class
    train_docs = [doc for i, doc in enumerate(documents) if labels[i] != unlearned_class]
    train_labels = [label for i, label in enumerate(labels) if label != unlearned_class]

    test_docs = [doc for i, doc in enumerate(documents) if labels[i] == unlearned_class]
    test_labels = [label for i, label in enumerate(labels) if label == unlearned_class]

    # Further split train_docs into 85% test and 15% train
    split_index = int(0.85 * len(train_docs))
    new_test_docs = train_docs[:split_index]  # New test set (from remaining classes)
    new_test_labels = train_labels[:split_index]
    
    new_train_docs = train_docs[split_index:]  # Training set
    new_train_labels = train_labels[split_index:]

    # Final test set includes:
    # - 85% of remaining classes
    # - 100% of unlearned class documents
    final_test_docs = new_test_docs + test_docs
    final_test_labels = new_test_labels + test_labels

    # Create word graph
    word_graph = create_word_graph(new_train_docs)

    # Run PageRank to get top words
    top_words = run_pagerank(word_graph, n=80000)

    # Build word-document graph
    graph_of_docs = build_word_document_graph(new_train_docs, new_train_labels, top_words)

    # Create class importance nodes
    class_importance = create_class_importance_nodes(graph_of_docs, new_train_labels)

    # Train classifier
    classify_document = train_classifier(class_importance, top_words)

    # Predict and evaluate on the final test set
    y_pred = [classify_document(doc) for doc in final_test_docs]
    accuracy_all = accuracy_score(final_test_labels, y_pred)
    print(f"Gold Standard - Accuracy on overall test set: {accuracy_all * 100:.2f}%")

    # Accuracy excluding the unlearned class
    valid_indices = [i for i, label in enumerate(final_test_labels) if label != unlearned_class]
    y_test_filtered = [final_test_labels[i] for i in valid_indices]
    y_pred_filtered = [y_pred[i] for i in valid_indices]
    accuracy_excl = accuracy_score(y_test_filtered, y_pred_filtered)

    print(f"Gold Standard - Accuracy excluding class {unlearned_class}: {accuracy_excl * 100:.2f}%")
    
    # Distribution of classifications for the unlearned class
    unlearned_class_indices = [i for i, label in enumerate(final_test_labels) if label == unlearned_class]
    y_pred_unlearned = [y_pred[i] for i in unlearned_class_indices]
    ...
    total_unlearned = sum(distribution)
    distribution_percent = [round((count / total_unlearned * 100), 2) if total_unlearned > 0 else 0 for count in distribution]

    print(f"Gold Standard - Distribution of classifications for class {unlearned_class} (as percentages):", distribution_percent)

    return distribution_percent

def compare_kl_divergence(main_distribution, gold_distribution):
    """Computes KL-divergence between the classification distributions of main and gold standard runs."""
    main_prob = np.array(main_distribution) / 100  # Convert to probabilities
    gold_prob = np.array(gold_distribution) / 100

    # Avoid log(0) errors by replacing 0 with a small epsilon
    epsilon = 1e-10
    main_prob = np.clip(main_prob, epsilon, 1)
    gold_prob = np.clip(gold_prob, epsilon, 1)

    kl_div = entropy(main_prob, gold_prob)
    print(f"KL-Divergence between main and gold standard: {kl_div:.4f}")
    return kl_div

# Run the main function
if __name__ == "__main__":
    unlearned_class = random.randint(0, 19)
    print(f"Selected class for unlearning: {unlearned_class}")
    main_distribution = (main(unlearned_class))
    gold_distribution = gold_standard(unlearned_class) 
    compare_kl_divergence(main_distribution, gold_distribution)