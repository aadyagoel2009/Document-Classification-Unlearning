...
def create_class_importance_nodes(graph_of_docs, labels, alpha=100):
...
def main():
    ...
    # --- Existing Distribution Comparisons (for each retained class) ---
    ...
    unlearned_confidences = test_dataset_20D[:, class_to_zero]
    overall_confidences = test_dataset_20D.flatten()  # all confidence scores across all documents and classes
    stat, p_value = ks_2samp(unlearned_confidences, overall_confidences)
    print("\nOverall KS test comparing unlearned class confidences to overall confidence distribution:")
    print(f"KS test statistic: {stat:.4f}, p-value: {p_value:.4f}")
    
    # --- New Block: Compare Target Classes for Unlearned Documents ---
    # Identify indices where the true label is the unlearned class.
    unlearned_indices = [i for i, label in enumerate(y_test_labels) if label == class_to_zero]
    if len(unlearned_indices) > 0:
        # Get predicted labels for these unlearned documents.
        unlearned_pred_labels = [y_pred[i] for i in unlearned_indices]
        # Count the frequency of predicted classes and take the top 2-3 classes.
        target_counts = Counter(unlearned_pred_labels)
        top_target_classes = [cls for cls, count in target_counts.most_common(3)]
        print(f"\nTop target classes for documents originally in class {class_to_zero}: {top_target_classes}")
        # For each target class, compare distributions.
        for target_class in top_target_classes:
            # Group 1: All documents predicted as target_class.
            group_all = [test_dataset_20D[i][target_class] for i in range(len(y_pred)) if y_pred[i] == target_class]
            # Group 2: Documents originally unlearned (true label equals class_to_zero) and predicted as target_class.
            group_unlearned = [test_dataset_20D[i][target_class] for i in unlearned_indices if y_pred[i] == target_class]
            if len(group_all) > 0 and len(group_unlearned) > 0:
                stat, p_value = ks_2samp(group_all, group_unlearned)
                print(f"\nTarget class {target_class}:")
                print(f"KS test statistic: {stat:.4f}, p-value: {p_value:.4f}")
                print(f"Group ALL (n={len(group_all)}): mean={np.mean(group_all):.4f}, std={np.std(group_all):.4f}")
                print(f"Group Unlearned (n={len(group_unlearned)}): mean={np.mean(group_unlearned):.4f}, std={np.std(group_unlearned):.4f}")
            else:
                print(f"\nTarget class {target_class}: Not enough data for KS test.")
    else:
        print("\nNo test instances originally belonged to the unlearned class; cannot compare target class distributions.")
