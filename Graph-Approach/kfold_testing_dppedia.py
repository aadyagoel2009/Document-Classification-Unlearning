...
def run_kfold(k, random_state = 42):

    ds = load_dataset("dbpedia_14")
    all_docs   = np.array(ds["train"]["content"] + ds["test"]["content"])
    all_labels = np.array(ds["train"]["label"]   + ds["test"]["label"])
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(all_docs, all_labels), 1):
        print(f"\n===== Fold {fold}/{k} =====")
        train_docs   = all_docs[train_idx].tolist()
        train_labels = all_labels[train_idx].tolist()
        test_docs  = all_docs[val_idx].tolist()
        test_labels= all_labels[val_idx].tolist()
