...
def create_class_importance_nodes(graph_of_docs, labels, alpha=100000):
...
def evaluate_kmeans_clusters(doc_vectors, true_labels, unlearned_class, n_clusters=20):
    ...
    for cluster in range(n_clusters):
        ...
        unlearned_counts.append(unlearned_in_cluster)
    ...
    return cluster_stats, total_unlearned, unlearned_counts, cluster_labels

def evaluate_privacy(doc_vectors, true_labels, unlearned_class, n_clusters=20):
    """
    Evaluates privacy by first clustering the document vectors using KMeans (via evaluate_kmeans_clusters)
    and then computing the entropy of the distribution of unlearned class documents among clusters.
    
    Higher entropy indicates that unlearned documents are distributed more uniformly (i.e. better privacy).
    """
    cluster_stats, total_unlearned, unlearned_counts, cluster_labels = evaluate_kmeans_clusters(
        doc_vectors, true_labels, unlearned_class, n_clusters
    )
    if total_unlearned > 0:
        # Calculate probabilities for unlearned documents in each cluster
        probs = [count / total_unlearned for count in unlearned_counts if count > 0]
        entropy_value = -sum(p * log2(p) for p in probs if p > 0)
    else:
        entropy_value = 0
    print(f"Privacy Entropy: {entropy_value:.4f}")
    return entropy_value, cluster_stats
...
def main():
    ...
    print("\n--- Privacy Evaluation BEFORE Unlearning ---")
    evaluate_privacy(test_dataset_20D, y_test_labels, class_to_zero)
    ...
    evaluate_kmeans_clusters(test_dataset_20D, y_test_labels, class_to_zero)
    print("\n--- Privacy Evaluation AFTER Unlearning ---")
    evaluate_privacy(test_dataset_20D, y_test_labels, class_to_zero)
