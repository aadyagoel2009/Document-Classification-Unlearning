...
def analyze_centroid_shift(centroids_before, centroids_after, unlearned_class):
    """Analyze how the unlearned class centroid moves relative to other centroids."""
    class_centroids_before = centroids_before["class_centroids"]
    class_centroids_after = centroids_after["class_centroids"]

    if unlearned_class not in class_centroids_after:
        print(f"Class {unlearned_class} centroid was removed after unlearning.")
        return

    distances_before = {}
    distances_after = {}

    for cls, centroid in class_centroids_before.items():
        if cls != unlearned_class:  # Compare to all other classes
            distances_before[cls] = euclidean(class_centroids_before[unlearned_class], centroid)
            distances_after[cls] = euclidean(class_centroids_after[unlearned_class], centroid)

    print("\nCentroid Shift Analysis:")
    for cls in distances_before:
        shift = distances_before[cls] - distances_after[cls]
        direction = "closer" if shift > 0 else "farther"
        print(f"Distance to Class {cls}: Moved {direction} by {abs(shift):.4f}")

    # Find closest class after unlearning
    closest_class = min(distances_after, key=distances_after.get)
    print(f"\nAfter unlearning, Class {unlearned_class} is now closest to Class {closest_class}.")
...
def main():
    ...
    analyze_centroid_shift(centroids_before, centroids_after, class_to_zero)
