...
# --------------------------
# Graph-Based and Unlearning Functions
# --------------------------
...
def custom_loss(outputs, true_labels, unlearned_class, lambda_align=0.01):
    """
    Combined loss: standard cross entropy plus a small alignment term.
    The alignment term encourages the unlearned class's probability to be near its batch mean.
    """
...
def compute_doc_representation(documents, top_words, class_importance):
    """
    Compute a 20-dimensional document representation for each document.
    Each vector is the normalized sum of the class importance vectors for its words.
    """
    representations = []
    for doc in documents:
        words = set(clean_data(doc))
...
# --------------------------
# Main Function: Two-Stage Training
# --------------------------
...
def main():
    ...
    # Select a random class to unlearn
    class_to_zero = random.randint(0, 19)
    print(f"Unlearned Class: {class_to_zero}")

    # (Optional) TF-IDF step for efficiency, though not used further here.
    vectorizer = TfidfVectorizer(max_features=5000)
    vectorizer.fit(train_dataset)

    # Build word graph and compute keyword importance via PageRank
    word_graph = create_word_graph(train_dataset)
    top_words = run_pagerank(word_graph, n=80000)

    # Build word-document graph and compute class importance nodes
    ...
    print(f"Base Graph-Based Classifier Accuracy (before calibration): {base_accuracy * 100:.2f}%")
    
    # Unlearning process: remove high-importance words and zero-out the unlearned class
    ...
    # ------------------ Stage 2: Calibration (Backpropagation) ------------------
    # Compute 20D representations for train and test sets (these are now fixed)
    ...
    # Convert representations and labels into PyTorch tensors
    ...
    # Define the calibration network and optimizer
    calibration_model = CalibrationLayer(input_dim=20, output_dim=20)
    optimizer = optim.Adam(calibration_model.parameters(), lr=0.001)
    num_epochs = 20
    lambda_align = 0.01  # Use a small weight to preserve the high accuracy

    # Train calibration network (Stage 2)
    calibration_model.train()
    for epoch in range(num_epochs):
        optimizer.zero_grad()
        outputs = calibration_model(X_train)
        loss = custom_loss(outputs, y_train, unlearned_class=class_to_zero, lambda_align=lambda_align)
        loss.backward()
        optimizer.step()
        print(f"Epoch {epoch+1}/{num_epochs}, Loss: {loss.item():.4f}")

    # Evaluate calibration network on the test set
    calibration_model.eval()
    with torch.no_grad():
        calibrated_outputs = calibration_model(X_test)
        predicted_labels = torch.argmax(calibrated_outputs, dim=1)
        calibrated_accuracy = (predicted_labels == y_test).float().mean().item()
    print(f"\nCalibrated Accuracy on test data: {calibrated_accuracy * 100:.2f}%")

    # Perform KS test on calibrated outputs: compare unlearned class distribution vs. overall
    unlearned_calibrated = calibrated_outputs[:, class_to_zero].cpu().numpy()
    overall_calibrated = calibrated_outputs.cpu().numpy().flatten()
    stat, p_value = ks_2samp(unlearned_calibrated, overall_calibrated)
    print(f"\nKS test after calibration: Statistic: {stat:.4f}, p-value: {p_value:.4f}")
    
    # Also, print some summary statistics
    print("\nAfter calibration, unlearned class probability statistics:")
    print(f"Mean: {np.mean(unlearned_calibrated):.4f}, Std: {np.std(unlearned_calibrated):.4f}")
    print("Overall calibrated probabilities statistics:")
    print(f"Mean: {np.mean(overall_calibrated):.4f}, Std: {np.std(overall_calibrated):.4f}")
