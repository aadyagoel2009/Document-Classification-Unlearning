...
def compute_largest_distance(class_importance, documents, labels, top_words):
    """
    Compute centroids for each class and return the farthest (largest) distance
    from the centroid for each class along with overall centroid info.
    """
    ...
    for idx, doc in enumerate(documents):
        words = set(clean_data(doc))
        doc_vector = np.zeros(20)  # Feature vector for 20 classes
        for word in words:
            if word in top_words and word in class_importance:
                doc_vector += np.array(class_importance[word])
        if np.sum(doc_vector) > 0:
    ...
    # Compute largest distances from centroids
    class_largest_distances = {}
    for cls, vectors in class_vectors.items():
        distances = np.linalg.norm(np.array(vectors) - class_centroids[cls], axis=1)
        class_largest_distances[cls] = np.max(distances)
    
    dataset_distances = np.linalg.norm(np.array(dataset_vectors) - dataset_centroid, axis=1)
    dataset_largest_distance = np.max(dataset_distances)

    return {
        "class_centroids": class_centroids,
        "dataset_centroid": dataset_centroid,
        "class_largest_distances": class_largest_distances,
        "dataset_largest_distance": dataset_largest_distance
    }

def compute_unlearned_post_class_largest_distance(unlearned_vectors, unlearned_pred_labels, class_centroids):
    """
    For each class, compute the largest Euclidean distance of the re-assigned unlearned 
    documents from that class centroid.
    """
    largest_distances = {}
    lengths = {}
    for cls in range(20):
        # Filter re-assigned unlearned document vectors for which the predicted label equals cls
        cls_vectors = [vec for vec, pred in zip(unlearned_vectors, unlearned_pred_labels) if pred == cls]
        centroid = class_centroids.get(cls)
        if centroid is not None and cls_vectors:
            cls_distances = [np.linalg.norm(np.array(vec) - np.array(centroid)) for vec in cls_vectors]
            largest_distances[cls] = np.max(cls_distances)
        else:
            largest_distances[cls] = None
        lengths[cls] = len(cls_vectors)
    return largest_distances, lengths
...
def main():
    ...
    largest_before = compute_largest_distance(class_importance, test_dataset, y_test_labels, top_words)
    distance_before = largest_before["class_largest_distances"].get(class_to_zero, None)
    print(f"Largest distance for unlearned class {class_to_zero} BEFORE unlearning: {distance_before:.4f}")

    largest_after = compute_largest_distance(class_importance, test_dataset, y_test_labels, top_words)
    distance_after = largest_after["class_largest_distances"].get(class_to_zero, None)
    print(f"Largest distance for unlearned class {class_to_zero} AFTER unlearning: {distance_after:.4f}")
    ...
    # Use the centroids from your post-unlearning computation:
    largest_unlearned, number_docs = compute_unlearned_post_class_largest_distance(unlearned_vectors, unlearned_pred_labels, largest_after["class_centroids"])
    for cls in range(20):
        if largest_unlearned[cls] is not None:
            print(f"Largest distance for re-assigned unlearned documents in class {cls}: {largest_unlearned[cls]:.4f}")
            print(f"Number of re-assigned docs in class {cls}: {number_docs[cls]}")
            print(f"Largest distance for existing documents in class {cls}: {largest_after['class_largest_distances'].get(cls, 0):.4f}")
        else:
            print(f"No unlearned documents were re-assigned to class {cls}.")
        print("")
