...
def tost_ind(sample1, sample2, low, high):
    """
    Perform the TOST procedure for two independent samples.
    
    Parameters:
        sample1: 1D numpy array of the first sample (e.g., unlearned confidences)
        sample2: 1D numpy array of the second sample (e.g., overall confidences)
        low: Lower equivalence bound (typically a negative value)
        high: Upper equivalence bound (typically a positive value)
    
    Returns:
        t1: t-statistic for the lower bound test
        t2: t-statistic for the upper bound test
        pval: The maximum p-value of the two one-sided tests (TOST p-value)
        df: Degrees of freedom (using Welch’s approximation)
    """
    n1 = len(sample1)
    n2 = len(sample2)
    ...
    # Lower bound test: H0: mean1 - mean2 <= low  vs  H1: mean1 - mean2 > low
    t1 = ((mean1 - mean2) - low) / se
    pval1 = 1 - stats.t.cdf(t1, df)
    
    # Upper bound test: H0: mean1 - mean2 >= high  vs  H1: mean1 - mean2 < high
    t2 = ((mean1 - mean2) - high) / se
    pval2 = stats.t.cdf(t2, df)
    
    pval = max(pval1, pval2)
    return t1, t2, pval, df
...
def main():
    ...
    # Additional TOST test: Compare the unlearned class's confidence distribution to the overall distribution.
    # Define an equivalence margin (delta). Adjust this value based on what you consider to be a negligible difference.
    delta = 0.05
    unlearned_confidences = test_dataset_20D[:, class_to_zero]
    overall_confidences = test_dataset_20D.flatten()  # overall distribution across all documents and classes
    
    t_lower, t_upper, p_tost, df = tost_ind(unlearned_confidences, overall_confidences, -delta, delta)
    print("\nTOST equivalence test comparing unlearned class confidences to overall confidence distribution:")
    print(f"t-statistic (lower bound test): {t_lower:.4f}")
    print(f"t-statistic (upper bound test): {t_upper:.4f}")
    print(f"TOST p-value: {p_tost:.4f} (with equivalence margin ±{delta})")
