...
def create_class_importance_nodes(graph_of_docs, labels, alpha=1):
...
def compute_kl_divergence(y_pred, y_true, unlearned_class):
    """
    Compute the KL divergence between the unlearned class's distribution and the closest class distribution.

    Args:
        y_pred (list[int]): Predicted class labels for each document.
        y_true (list[int]): True class labels for each document.
        unlearned_class (int): The class that was unlearned.

    Returns:
        float: KL divergence between the unlearned class's distribution and the closest class's distribution.
    """
    ...
    unlearned_dist = [(count / total_unlearned) if total_unlearned > 0 else 0 for count in unlearned_dist]
    ...
    overall_dist = [(count / total_predictions) if total_predictions > 0 else 0 for count in overall_dist]

    # Determine the closest class based on distribution percentages
    closest_class = overall_dist.index(max(overall_dist))

    # Predicted distribution for the closest class
    closest_class_indices = [i for i, label in enumerate(y_true) if label == closest_class]
    closest_class_predictions = [y_pred[i] for i in closest_class_indices]

    closest_class_dist = [0] * 20
    for pred in closest_class_predictions:
        closest_class_dist[pred] += 1

    # Normalize to get probabilities
    total_closest_class = sum(closest_class_dist)
    closest_class_dist = [(count / total_closest_class) if total_closest_class > 0 else 0 for count in closest_class_dist]

    # Smoothing to avoid zeros
    epsilon = 1e-10
    unlearned_dist = [p + epsilon for p in unlearned_dist]
    closest_class_dist = [q + epsilon for q in closest_class_dist]

    # KL Divergence using relative entropy
    kl_divergence = entropy(unlearned_dist, closest_class_dist)

    # Debugging: Print intermediate distributions
    print("Unlearned Distribution:", unlearned_dist)
    print("Closest Class Distribution:", closest_class_dist)
